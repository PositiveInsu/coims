...
export class ManageFeeComponent implements OnInit{
  ...
  ngOnInit(): void {
    this._mainService.changeMenu( this._mainService.leftMenuFactory.FEE);
    this.initValuables();
    this.initForm();
  ...
  private getDollarFormControl( caseType?:CaseType): FormControl{
    return new FormControl( this.getDollar(caseType.getFee()), {updateOn: 'blur', validators: Validators.compose([ Validators.required, Validators.pattern( SignupValidator.caseFeeDollarExp)])});
  }

  private getCentFormControl( caseType?:CaseType): FormControl{
    return new FormControl( this.getCent(caseType.getFee()), {updateOn: 'blur', validators: Validators.compose([ Validators.required, Validators.pattern( SignupValidator.caseFeeCentExp)])});
  }
  ...
  private submit( key: string){
    ...
    this.modifyCaseFee( modifyCaseTypeList);
    this.changeMode( key);
  }
  ...
  private setCaseTypeFeeValue(caseTypeList: Array<CaseType>, formGroup: FormGroup, modifyCaseTypeList: Array<CaseType>): Array<CaseType>{
    for( let caseType of caseTypeList){
      if( caseType.getHasChild()){
        ...
        modifyCaseTypeList.push( this.getCaseTypeFeeValue( caseType, formGroup));
      }
    }
    return modifyCaseTypeList;
  }

  private getCaseTypeFeeValue( caseType: CaseType, formGroup: FormGroup): CaseType {
    ...
    modifyCaseType.setCompanyId( caseType.getCompanyId());
    modifyCaseType.setFee( Number( dollar + '.' + cents));
  ...
  private modifyCaseFee( modifyCaseTypeList: Array<CaseType>) {
    this._http.post( this._urlService.modifyCaseFee, JSON.stringify( modifyCaseTypeList), {headers: this._urlService.headerJson})
  ...
  private downloadFeeList(){
    this._http.get( this._urlService.caseTypeFeeExcel, { headers: this._urlService.headerStream, params: new HttpParams().set( "id", this._commonService.user.getCompany().getId().toString()), responseType: 'blob' })
  ...
  private afterDownload( obj: Blob) {
    ...
    a.href = url;
    a.download = 'CaseFee_' + this._commonService.user.getCompany().getName() + '.xlsx';
    a.click();
